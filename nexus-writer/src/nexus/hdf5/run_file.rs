...
use super::hdf5_writer::{add_new_group_to, set_attribute_list_to, set_group_nx_class};
use crate::nexus::{
    nexus_class as NX,
    DATETIME_FORMAT,
    TIMESTAMP_FORMAT,
...
impl RunFile {
    ...
    pub(crate) fn init(&mut self, parameters: &RunParameters) -> Result<()> {
        ...
        self.run_number.write_scalar(&parameters.run_number).unwrap();
        ...
        let start_time = DateTime::<Utc>::from_timestamp_millis(parameters.collect_from as i64)
            .ok_or(anyhow!("Cannot create start_time from {0}",&parameters.collect_from))?
            .format(DATETIME_FORMAT)
            .to_string();
        self.start_time.write_scalar(&start_time.parse::<VarLenUnicode>()?).unwrap();

        self.name.write_scalar(&parameters.run_name.parse::<VarLenUnicode>()?).unwrap();
        self.title.write_scalar(&"".parse::<VarLenUnicode>()?).unwrap();

        self.instrument_name.write_scalar(&parameters.instrument_name.parse::<VarLenUnicode>()?).unwrap();

        self.period_number.write_scalar(&parameters.num_periods).unwrap();
        self.period_type.resize(parameters.num_periods as usize).unwrap();
        self.period_type.write_raw(&vec![1; parameters.num_periods as usize]).unwrap();
    ...
    pub(crate) fn ensure_end_time_is_set(&mut self, parameters: &RunParameters, message : &GenericEventMessage) -> Result<()> {
        let end_ms = {
    ...
    pub(crate) fn push_message(&mut self, parameters: &RunParameters, message : &GenericEventMessage) -> Result<()> {
