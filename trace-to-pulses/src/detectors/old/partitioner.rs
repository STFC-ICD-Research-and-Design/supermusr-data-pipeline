...
use crate::window::{smoothing_window, smoothing_window::SmoothingWindow, Window};
...
impl<V, D> EventData for PartitionData<V, D>
where
    V: Default + Clone + Debug + 'static,
    D: EventData,
{
...
impl<V, D> Display for PartitionData<V, D>
where
...
pub struct Partitioner<Det, Evt, V, D>
where
...
impl<Det, Evt, V, D> Partitioner<Det, Evt, V, D>
where
    Det: Detector<ValueType = V, TimeType = Real, EventType = Evt>,
    Evt: EventWithData<DataType = D>,
    V: Default + Clone + Debug,
...
impl<Det, Evt, V, D> Clone for Partitioner<Det, Evt, V, D>
where
...
impl<Det, Evt, V, D> Detector for Partitioner<Det, Evt, V, D>
where
    Det: Detector<ValueType = V, TimeType = Real, EventType = Evt>,
    Evt: EventWithData<DataType = D>,
    V: Default + Clone + Debug + 'static,
