...
use crate::{events::event::Event, Detector};
use std::fmt::Debug;
use std::fmt::Display;

//#[derive(Default)]
pub struct TracePartition<I, D>
where
    I: Iterator,
...
impl<I, D> Clone for TracePartition<I, D>
where
    I: Iterator + Clone,
    I::Item: TraceData<TimeType = D::TimeType, ValueType = D::ValueType>,
    D: Detector,
...
impl<I, D> Debug for TracePartition<I, D>
where
    I: Iterator + Debug,
    I::Item: TraceData<TimeType = D::TimeType, ValueType = D::ValueType>,
    D: Detector,
...
impl<I, D> Display for TracePartition<I, D>
where
    I: Iterator + Clone + Debug,
    I::Item: TraceData<TimeType = D::TimeType, ValueType = D::ValueType>,
    D: Detector,
    ...
    D::DataType: Clone,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!(
            "event:{0}, from {1:?} to {2}",
            self.event, self.iter, self.length
        ))
    }
}

#[derive(Default)]
pub struct SubPartitionIter<I>
where
    I: Iterator,
{
    iter: I,
...
impl<I> Iterator for SubPartitionIter<I>
where
    I: Iterator,
{
    type Item = I::Item;
...
impl<I, D> Iterator for PartitionIter<I, D>
where
    I: Iterator + Clone + Debug,
    I::Item: TraceData<TimeType = D::TimeType, ValueType = D::ValueType>,
    D: Detector,
...
impl<I, D> PartitionFilter<I, D> for I
where
    I: Iterator,
    I::Item: TraceData<TimeType = D::TimeType, ValueType = D::ValueType>,
    D: Detector,
...
#[cfg(test)]
mod tests {
    use crate::{peak_detector::PeakDetector, Real};
