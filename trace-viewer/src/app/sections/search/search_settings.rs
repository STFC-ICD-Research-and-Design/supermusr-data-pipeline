        ...
        <Section name = "Search">
            <Panel>
                <ControlBoxWithLabel name = "search-mode" label = "Search Mode: ">
                    <select name = "search-mode" on:change:target = move |ev| set_search_mode.set(ev.target().value().parse().expect("SearchMode value should parse, this should never fail.")) >
                        <For
