...
#[component]
pub(crate) fn Display(selected_trace: impl Fn() -> Option<Vec<u16>> + Send + 'static) -> impl IntoView {
    let node_refs = use_context::<DisplaySettingsNodeRefs>()
        .expect("");
    move ||
    selected_trace().map(|trace| {
        let data = Signal::derive(move ||trace.iter().enumerate().map(|(x,y)|(x as f64, *y as f64)).collect::<Vec<_>>());
        let width = node_refs.width_ref.get().and_then(|node_ref|node_ref.value().parse().ok()).unwrap_or(800.0);
        let height = node_refs.height_ref.get().and_then(|node_ref|node_ref.value().parse().ok()).unwrap_or(600.0);
        view! {
            <Panel>
                <div></div>
