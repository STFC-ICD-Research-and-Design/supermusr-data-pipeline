...
pub(crate) struct TextDrawingBackend(pub Vec<PixelState>);
...
impl DrawingBackend for TextDrawingBackend {
    ...
    fn present(&mut self) -> Result<(), DrawingErrorKind<std::io::Error>> {
        let s = self.get_size();
        let s = (s.0 as usize, s.1 as usize);
        for r in 0..s.1 {
    ...
    fn draw_pixel(
        ...
        if color.alpha > 0.3 {
            self.0[(pos.1 * WIDTH + pos.0) as usize].update(PixelState::Pixel);
        }
        Ok(())
    }
    ...
    fn draw_line<S: BackendStyle>(
        ...
        if from.0 == to.0 {
            ...
            for y in y0..y1 {
                self.0[(y * WIDTH + x) as usize].update(PixelState::VLine);
            }
            return Ok(());
        }
        ...
        if from.1 == to.1 {
            ...
            for x in x0..x1 {
                self.0[(y * WIDTH + x) as usize].update(PixelState::HLine);
            }
            return Ok(());
        }
    ...
    fn draw_text<S: BackendTextStyle>(
        ...
        let offset = (pos.1 + dy).max(0) * WIDTH + (pos.0 + dx).max(0);
