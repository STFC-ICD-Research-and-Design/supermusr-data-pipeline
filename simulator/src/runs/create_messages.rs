use super::{
    runlog, sample_environment, AlarmData, LogError, RunLogData, SampleEnvData, SampleEnvTimestamp, Start, Stop
};
...
pub(crate) async fn create_runlog_command(
    ...
    let run_log_args = f144_LogDataArgs {
        ...
        timestamp: timestamp
            .signed_duration_since(DateTime::UNIX_EPOCH)
            .num_nanoseconds()
            .ok_or(LogError::TimestampToNanos(timestamp))?,
        value_type,
...
pub(crate) async fn create_sample_environment_command(
    ...
    let packet_timestamp = packet_timestamp
        .signed_duration_since(DateTime::UNIX_EPOCH)
        .num_nanoseconds()
        .ok_or(LogError::TimestampToNanos(packet_timestamp))?;
