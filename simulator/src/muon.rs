...
impl Muon {
    pub(crate) fn sample(template: &json::PulseAttributes, frame_index: usize) -> Self {
        match template {
            ...
            json::PulseAttributes::Biexp {
                ...
                let coef = peak_height / (f64::powf(ratio, 1.0/ratio - 1.0) - f64::powf(ratio, 1.0 - ratio));
                let peak_time = f64::ln(f64::powf(ratio , decay*rise/(decay - rise)));
                Self::Biexp {
                    start, decay, rise, peak_height, coef, peak_time
                }
                /*
                f(t) = A(e^{-t/d} - e^{-t/r})
    ...
    pub(crate) fn time(&self) -> Time {
        (match self {
            Self::Flat { start, stop: _, amplitude: _ } => *start,
            Self::Triangular { start: _, peak_time, stop: _, amplitude: _ } => *peak_time,
            Self::Gaussian { mean, sd: _, peak_amplitude: _ } => *mean,
            Self::Biexp { start, decay: _, rise: _, peak_height: _, coef: _, peak_time } => *start + *peak_time/2.0,
        }) as Time
    }
    pub(crate) fn intensity(&self) -> Intensity {
        *match self {
            Self::Flat { start: _, stop: _, amplitude, } => amplitude,
            Self::Triangular { start: _, peak_time: _, stop: _, amplitude } => amplitude,
            Self::Gaussian { mean: _, sd: _, peak_amplitude } => peak_amplitude,
            Self::Biexp { start: _, decay: _, rise: _, peak_height, coef: _, peak_time: _ } => peak_height,
        } as Intensity
    }
