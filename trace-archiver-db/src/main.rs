...
async fn main() -> Result<()> {
    ...
    if let Some(Mode::BenchmarkLocal(bmk)) = cli.mode {
        ...
        let arg_ranges = ArgRanges::from_option_or_env(bmk.num_samples_range, "BENCHMARK_NUM_SAMPLES_RANGE");
        let num_repeats : usize = unwrap_num_or_env_var(bmk.num_repeats, "BENCHMARK_REPEATS");

        let results = benchmark_local(tdengine, arg_ranges, num_channels, num_repeats, bmk.show_output).await;
    ...
    let consumer = match redpanda_engine::new_consumer(&redpanda_builder, &topic) {
        Some(c) => c,
        None => {
            redpanda_engine::create_topic(&redpanda_builder, &topic).await.unwrap_or_else(|e|log_then_panic(format!("Cannot create topic {e}")));
            log::info!("Topic: {topic} created.");
            redpanda_engine::new_consumer(&redpanda_builder, &topic).unwrap_or_else(||log_then_panic_t(format!("Cannot subscribe, reason unknown.")))
        },
    };
    ...
    if let Some(Mode::BenchmarkKafka(bmk)) = cli.mode {
        ...
        let results = benchmark_kafka(tdengine, num_repeats*parameter_space_size, consumer, bmk.show_output).await;
...
#[cfg(feature = "benchmark")]
async fn benchmark_local(mut tdengine : TDEngine, arg_ranges : ArgRanges, num_channels : usize, num_repeats : usize, show_output : bool) -> Results {
...
#[cfg(feature = "benchmark")]
async fn benchmark_kafka(mut tdengine : TDEngine, num_messages : usize, consumer : RedpandaConsumer, show_output : bool) -> Results {
