...
use anyhow::Result;
use linregress::{
    FormulaRegressionBuilder, RegressionData, RegressionDataBuilder, RegressionModel,
};
...
pub fn create_model(
    data: &RegressionData,
    formula: &str,
) -> Result<RegressionModel, linregress::Error> {
...
fn print_multilin_reg_coef_stats(model: &RegressionModel, index: usize) {
    print!(
        "(p = {0:8.3e}, std-err = {1:8.5})",
        model.p_values()[index],
        model.se()[index] / 1000.0
    );
}
